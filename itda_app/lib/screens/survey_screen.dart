...
places = np.array([[ 
  ...
  PersonaResult _buildPersonaResult() {
    ...
    if (foodie >= 0.8)      badges.add('🍽 미식 탐험가');
    if (culture >= 0.8)     badges.add('🎭 전시·공연 러버');
    if (activity >= 0.8)    badges.add('🏃 액티비티 러버');
    if (nature >= 0.8)      badges.add('🌿 자연 힐링러');
    if (craft >= 0.8)       badges.add('🧵 공방·체험 좋아함');
    if (shoppingLv >= 0.8)  badges.add('🛍 쇼핑 코스 선호');

    if (romanticLv >= 0.8)  badges.add('💘 로맨틱 무드');
    if (trendyLv >= 0.8)    badges.add('🔥 트렌디 스팟 선호');
    if (privateLv >= 0.8 || quietLv >= 0.8) {
      badges.add('🤫 프라이빗 & 조용함 선호');
    }
    if (artisticLv >= 0.8)  badges.add('🖼 감각적인 공간 선호');
    if (energeticLv >= 0.8) badges.add('⚡ 활기찬 에너지');

    if (socialLv >= 0.8)    badges.add('🧑‍🤝‍🧑 소셜형 데이트');
    if (relaxLv >= 0.8)     badges.add('🧘 휴식·편안함 중시');
    if (photoLv >= 0.8)     badges.add('📸 인생샷 필수');
    if (viewLv >= 0.8)      badges.add('🌇 뷰 맛집 선호');
    ...
    // 1) 트렌디 + 미식 + 포토
    if (foodie >= 0.7 && trendyLv >= 0.7 && photoLv >= 0.6) {
      title = '트렌디 미식 & 포토 스팟형';
    ...
    // 2) 자연 + 휴식 (+뷰)
    else if (nature >= 0.7 && relaxLv >= 0.7 && indoorLv < 0.7) {
      title = '도심 탈출 힐링러';
    ...
    // 3) 액티비티 + 소셜 + 에너지
    else if (activity >= 0.7 && socialLv >= 0.7 && energeticLv >= 0.6) {
      title = '에너제틱 액티브 플레이어';
    ...
    // 4) 문화 + 아트 + 잔잔함
    else if (culture >= 0.7 && artisticLv >= 0.7 && quietLv >= 0.5) {
      title = '아트 & 스토리텔러형';
    ...
    // 5) 프라이빗 + 로맨틱
    else if (privateLv >= 0.7 && romanticLv >= 0.6) {
      title = '프라이빗 로맨티스트';
    ...
    // 6) 인도어 + 휴식 + 조용함
    else if (indoorLv >= 0.7 && relaxLv >= 0.7 && quietLv >= 0.6) {
      title = '실내 아지트 러버';
      tagline = '편안한 실내 공간에서 느긋하게 보내는 걸 선호해요.';
      tips.add('편안한 카페, 북카페, 라운지 바, 영화관 등을 1~2코스로 깊게 즐기게 해주세요.');
    }
    // 7) 트렌디 + 쇼핑 + 도심
    else if (trendyLv >= 0.7 && shoppingLv >= 0.6 && crowdLv >= 0.5) {
      title = '도심 핫스팟 러버';
    ...
    final trimmedBadges =
        badges.isEmpty ? ['🔍 밸런스형 취향'] : (badges.length > 6 ? badges.sublist(0, 6) : badges);
